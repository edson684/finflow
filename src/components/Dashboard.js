      ...
      <main style={{ marginLeft: isMobile ? 0 : sidebarWidth, flex: 1, padding: isMobile ? '20px 16px 80px' : '36px 36px 36px 40px' }}>
        ...
        {isMobile && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
            ...
            <button onClick={() => supabase.auth.signOut()} style={{ background: 'transparent', border: '1px solid #0f1f35', borderRadius: 8, color: '#475569', fontSize: 12, padding: '6px 12px', cursor: 'pointer' }}>Sair</button>
          </div>
        )}
            ...
            <div style={{ background: '#080f1e', border: '1px solid #0f1f35', borderRadius: 16, padding: '20px 20px' }}>
              ...
              {transactions.slice(0, 5).map(t => (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #0d1a2e' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 34, height: 34, borderRadius: 10, background: (t.type === 'income' ? '#38bdf8' : categoryColors[t.category] || '#6b7280') + '20', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15 }}>
                      {t.type === 'income' ? '↑' : '↓'}
                    </div>
                    <div>
                  ...
                  <span style={{ fontWeight: 700, fontSize: 14, color: t.type === 'income' ? '#22c55e' : '#f97316' }}>
                    {t.type === 'income' ? '+' : '-'}{formatCurrency(t.value)}
                  </span>
                </div>
              ))}
        ...
        {activeTab === 'goals' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
              ...
              <button onClick={() => setShowGoalModal(true)} style={{ background: 'linear-gradient(135deg, #06b6d4, #3b82f6)', color: '#fff', border: 'none', borderRadius: 10, padding: '11px 20px', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
                + Nova Meta
              </button>
            </div>
